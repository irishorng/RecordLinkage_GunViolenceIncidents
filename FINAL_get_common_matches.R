...
online_records <- read.csv("C:/Users/chiufang/Documents/URPS/RURPS/URPS_project3/GVA_online_combined_final_correct.csv")
onlinedata <- as.list(online_records$Incident.ID)

## the GVA and NVDRS matches gotten
## by using FINAL_processing_and_fastlink.R
merged <- read.csv("C:/Users/chiufang/Documents/URPS/RURPS/URPS_project3/final_draft_merged3.csv")
...
matched <- intersect(ourdata, onlinedata) 
...
merged_keep_matches <- merged_date_to_front[merged_date_to_front$Incident.ID %in% merged_matches,] 
...
merged_move_to_front <- merged %>% relocate(Incident.ID)
